# listing-alert-state-updates Specification

## Purpose
Keep dispatched Telegram alerts truthful after they fire: persist each alert's message id, watch the listing for a bounded window, and edit the original message in place on reserved flips and threshold-gated price drops — with a reply ping for large drops and a replayable audit trail of every edit.
## Requirements
