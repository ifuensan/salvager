...
**Promotion to `v1.0.0` is gated on production burn-in**, not feature completion. Recommended pinned tag for burn-in: `ghcr.io/ifuensan/salvager:0.4.3`. See [`CHANGELOG.md`](CHANGELOG.md) for the release notes and the `[1.0.0] — future` placeholder spelling out the promotion criteria.
...
1. ≥ 2 weeks of the current stable image (`:0.4.3` recommended) running continuously against live Wallapop + eBay.es traffic without unhandled crashes.
